...
class CustomSettingsDialogView(Drawable):
    def __init__(self, custom_settings_dialog):
        ...
        self._title_text = FONT_PRIMARY_XXLARGE.render(u'自定义设置', True, Color.WHITE)
        self._num_cpus_label_text = FONT_SECONDARY_MEDIUM.render(u'# CPU 数量', True, Color.WHITE)
        self._num_processes_label_text = FONT_SECONDARY_MEDIUM.render(u'# 启动时的进程数', True, Color.WHITE)
        self._num_ram_rows_label_text = FONT_SECONDARY_MEDIUM.render(u'# RAM 行数', True, Color.WHITE)
        self._new_process_probability_label_text = FONT_SECONDARY_MEDIUM.render(u'新建进程的概率', True, Color.WHITE)
        self._io_probability_label_text = FONT_SECONDARY_MEDIUM.render(u'I/O 事件发生概率', True, Color.WHITE)
